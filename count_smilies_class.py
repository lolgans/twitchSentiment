...
#     'twitchstandard': 'twitchstandard.txt',
#     'twitchothers': 'twitchothers.txt',
#     'afinn': 'AFINN-emoticon-8.txt',
#     'emoticons': 'EmoticonsSentimentLexicon.txt'
#     }
#
#
# class SmilieException(Exception):
#     """Base for exceptions raised in this module."""
#
#     pass
#
#
# class WordListReadingError(SmilieException):
...
# class Emo(object):
#
#     def __init__(self, smilies="twitchstandard", moreemos=True, combined="afinn", word_boundary=True):
...
#
#         """
#         smilie1 = LANGUAGE_TO_FILENAME[smilies]
#         full_smilie1 = self.full_filename(smilie1)
#
#         # Twitchsmilies
#         self._dict = self.read_word_file(full_smilie1)
#         regex_smilies = self.regex_from_tokens(
#             list(self._dict),
...
#         # Other Emoticons
#         if moreemos:
#             filename_emoticons = LANGUAGE_TO_FILENAME[combined]
...
#                 list(emoticons_and_score), word_boundary=True,
#                 capture=False)
#
#             # combined smilies and emoticon to regular expression
#             regex = '(' + regex_smilies + '|' + regex_emoticons + ')'
#
...
#
#         """
#         return join(dirname(__file__), 'smilies')
#
...
#
#         missing_words = []
#         sentence = set(text.split()) #.lower()
#         # print(sentence)
#         for word in sentence:
#             if word not in words:
#                 missing_words.append(word)
#         # print(missing_words)
#         # print(word for word in words if word not in sentence)
#         return words, missing_words
#
#     def split(self, text):
...
#         words, missingWords = self.find_all(text)
...
LANGUAGE_TO_FILENAME = {
    ...
    'twitchothers': 'twitchothers.txt',
    'afinn': 'AFINN-emoticon-8.txt',
    'emoticons': 'EmoticonSentimentLexicon.txt',
    'empty': 'empty.txt'
    }

foundWords = {}
foundEmos = {}
numOfFoundSentences = 0
...
class Emo(object):
    ...
    def __init__(self, language="twitchstandard", emoticons=False, secondemo="afinn", word_boundary=True):
        """Setup dictionary from data file.
        ...
        if emoticons:
            ...
            # Emoticons
            filename_emoticons = LANGUAGE_TO_FILENAME[secondemo]
            full_filename_emoticons = self.full_filename(filename_emoticons)
            emoticons_and_score = self.read_word_file(full_filename_emoticons)
            self._dict.update(emoticons_and_score)
            regex_emoticons = self.regex_from_tokens(
    ...
    def score_with_pattern(self, text):
        ...
        """
        # TODO: ":D" is not matched
        words = self.find_all(text) #, missingWords
        word_scores = (self._dict[word] for word in words)
        score = float(sum(word_scores))
        return score, words
